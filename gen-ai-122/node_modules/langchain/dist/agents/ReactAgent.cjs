...
var ReactAgent = class ReactAgent {
	...
	constructor(options, defaultConfig) {
		...
		*/
		const { state, input, output } = require_annotation.createAgentState(this.options.responseFormat !== void 0, this.options.stateSchema, this.options.middleware);
	...
	*/
	#createToolsRouter(shouldReturnDirect, exitNode) {
		return (state) => {
			...
			if (_langchain_core_messages.ToolMessage.isInstance(lastMessage) && lastMessage.name && shouldReturnDirect.has(lastMessage.name)) return this.options.responseFormat ? require_AgentNode.AGENT_NODE_NAME : exitNode;
