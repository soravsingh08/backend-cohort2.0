...
var ibm_exports = /* @__PURE__ */ require_runtime.__exportAll({ ChatWatsonx: () => ChatWatsonx });
...
function _convertToolToWatsonxTool(tools) {
	return tools.map((tool) => {
		...
		const parameters = (0, _langchain_core_utils_types.isInteropZodSchema)(tool.schema) ? (0, _langchain_core_utils_json_schema.toJsonSchema)(tool.schema) : tool.schema;
		return {
			type: "function",
			function: {
				name: tool.name,
...
const getToolCalls = (message, model) => {
	if ((0, _langchain_core_messages.isAIMessage)(message) && message.tool_calls?.length) return message.tool_calls.map((toolCall) => ({
		...toolCall,
...
function _convertDeltaToMessageChunk(helperIndex, delta, rawData, model, usage, defaultRole) {
	...
	switch (role) {
		case "user": return new _langchain_core_messages.HumanMessageChunk({ content });
		case "assistant": {
		...
		case "tool":
			if (rawToolCalls) return new _langchain_core_messages.ToolMessageChunk({
				content,
				additional_kwargs,
				tool_call_id: _convertToValidToolId(model ?? "", rawToolCalls[0].id)
			});
			return null;
		case "function": return new _langchain_core_messages.FunctionMessageChunk({
			content,
			additional_kwargs
		});
		default: return new _langchain_core_messages.ChatMessageChunk({
			content,
			role
		});
	}
...
var ChatWatsonx = class extends _langchain_core_language_models_chat_models.BaseChatModel {
	static lc_name() {
		return "ChatWatsonx";
	}
	...
	async completionWithRetry(callback, options) {
		...
		return options ? caller.callWithOptions({ signal: options.signal }, async () => callback()) : caller.call(async () => callback());
	}
	...
	async _generate(messages, options, runManager) {
		if (this.streaming) {
			...
			for await (const chunk of stream) {
				const usageMetadata = chunk.message?.usage_metadata;
				if (usageMetadata) {
	...
	async *_streamResponseChunks(messages, options, _runManager) {
		...
		for await (const chunk of stream) {
			...
			_runManager?.handleLLMNewToken(generationChunk.text, newTokenIndices, void 0, void 0, void 0, { chunk: generationChunk });
		}
	...
	withStructuredOutput(outputSchema, config) {
		...
		if (method === "jsonMode") {
			...
			if ((0, _langchain_core_utils_types.isInteropZodSchema)(schema)) {
				outputParser = _langchain_core_output_parsers.StructuredOutputParser.fromZodSchema(schema);
				outputFormatSchema = (0, _langchain_core_utils_json_schema.toJsonSchema)(schema);
		...
		} else if ((0, _langchain_core_utils_types.isInteropZodSchema)(schema)) {
